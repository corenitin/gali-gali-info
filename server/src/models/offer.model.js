...
offerSchema.pre("save", function (next) {
  ...
  switch (this.type) {
    case "percentage":
      if (
        typeof this.details.discount !== "number" ||
        this.details.discount <= 0 ||
        this.details.discount > 100
      ) {
        return next(
          new Error(
            "Percentage offer must have a discount value between 1 and 100."
          )
        );
      }
      break;
    ...
    case "fixed":
      if (
        ...
        return next(
          new Error("Fixed offer must have a valid discount amount.")
        );
      }
      break;
    ...
    case "conditional":
      if (
        typeof this.details.minPurchase !== "number" ||
        typeof this.details.discount !== "number"
      ) {
        return next(
          new Error(
            "Conditional offer must include `minPurchase` and `discount`."
          )
        );
      }
      break;
    ...
    case "buy":
      if (
        typeof this.details.buyQty !== "number" ||
        typeof this.details.getQty !== "number"
      ) {
